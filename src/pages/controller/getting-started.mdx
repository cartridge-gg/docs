...
The two main connectors serve fundamentally different deployment contexts.

### Choosing a Connector

| Aspect | ControllerConnector | SessionConnector |
|--------|---------------------|------------------|
| **Use when** | Building web apps | Building native/mobile apps |
| **Authentication** | Browser cookies & localStorage | Local keypair + browser redirect |
| **RPC configuration** | Via framework provider | Explicit in constructor |
| **Framework integration** | `starknet-react`, wallet standards | Standalone |
| **Redirect handling** | Not required | Required (`redirectUrl`) |

**Decision guide:**
- If your app runs in a browser and you're using `starknet-react` → **ControllerConnector**
- If your app is native (iOS, Android, React Native, Capacitor) or a mini-app (Telegram) → **SessionConnector**

The key difference is where the signing keys live.
With `ControllerConnector`, keys are managed by Controller's backend via browser cookies.
With `SessionConnector`, your app generates a local keypair and registers it with the user's Controller account through a browser redirect flow.
...
The `SessionConnector` enables native and mobile applications to create local sessions linked to a user's existing Controller account.
Unlike `ControllerConnector`, it generates a local keypair that signs transactions on behalf of the user.

**How it works:**
1. Your app generates a local keypair
2. User authenticates in a browser (opened via deep link)
3. Browser redirects back to your app with session credentials
4. Your app signs transactions locally using the private key

**When to use SessionConnector:**
- Mobile apps (React Native, Capacitor, native iOS/Android)
- Telegram mini-apps or Discord bots
- Any environment where browser cookies aren't available
...
:::info
Under-the-hood, `SessionConnector` returns a `SessionProvider`, an alternative Controller implementation designed for native applications.
For web application use-cases, use the regular `Controller` or `ControllerConnector` instead.
:::

Note that `SessionConnector` requires explicit `rpc` and `chainId` parameters because it operates outside a framework like `starknet-react` that would normally provide these.
...
- Display [Toast Notifications](/controller/notifications.md) for user feedback
