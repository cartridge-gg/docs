...
The following common dependencies are compatible with the latest version of Cartridge Controller (v0.11.2):

:::note
This ecosystem is evolving rapidly.
This information is accurate as of November 14, 2025.
:::

```ts
  "@cartridge/arcade": "0.0.2"
  "@cartridge/connector": "0.11.2"
...
### Migration Notes for v0.11.2

#### Storage Access API Enhancement

Version 0.11.2 introduces automatic Storage Access API integration with encrypted localStorage snapshots. This enhancement:

- **No breaking changes**: Existing applications continue to work without modification
- **Improved iOS support**: Better authentication flow reliability on iOS Safari
- **Enhanced third-party embedding**: More reliable operation when embedded in third-party contexts
- **Automatic encryption**: User state is now automatically encrypted using split-key architecture

Applications will automatically benefit from these improvements without requiring any code changes.
