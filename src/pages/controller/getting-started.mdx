...
To restrict the authentication options shown to users, pass `signupOptions` to the constructor:

```ts
import Controller from "@cartridge/controller";

// Only show Phantom EVM as an authentication option
const controller = new Controller({
...
// With custom redirect URL and preset theme
const controller = new Controller({
  preset: "mygame",
});
controller.open({
...
:::info
The `preset` theme is configured at construction time via the `Controller` constructor, not per-call.
...
To restrict the authentication methods shown to users, pass `signupOptions` to the `ControllerConnector` constructor:
...
You can use the `ControllerConnector` to cast to the wallet-standard compatible `WalletWithStarknetFeatures` interface:
