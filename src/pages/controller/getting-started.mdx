...
The ControllerConnector supports the [get-starknet wallet standard](https://get-starknet.vercel.app/), enabling integration with frontend libraries like `starknet-react` and generic libraries like `solid.js`. This provides a higher-level interface than the standard `StarknetWindowObject`.

### Using asWalletStandard()

You can convert a ControllerConnector to a wallet-standard compatible interface:
...
// Convert to wallet standard interface
...
:::warning
The `asWalletStandard()` method is an experimental feature and may contain bugs. Please [report any issues](https://github.com/cartridge-gg/controller/issues) you encounter.
:::

### Alternative Direct Casting

You can also use the `StarknetInjectedWallet` directly:
...
const controller = new Controller();
const walletStandard = new StarknetInjectedWallet(controller);
```

The wallet standard interface provides these key methods:
- `StandardConnect` (similar to `connect`)
- `StandardDisconnect` (similar to `disconnect`) 
- `StandardEvents` (similar to `on`)
- `StarknetWalletApi` (similar to `request`)
...
This hybrid mode runs the keychain and examples locally while connecting to production APIs. This is useful for:

- Testing your application against production data
- Debugging issues that only occur with production configurations
- Validating your integration before deployment
