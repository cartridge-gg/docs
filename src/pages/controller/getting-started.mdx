...
The ControllerConnector supports the [get-starknet wallet standard](https://get-starknet.vercel.app/), a higher-level interface than the`StarknetWindowObject` which enables alternative integration with libraries like `starknet-react` and `solid.js`.

### Using asWalletStandard()

You can use ControllerConnector to cast Controller to the wallet-standard compatible interface `WalletWithStarknetFeatures`

```tsx
import { ControllerConnector } from "@cartridge/connector";

const controllerConnector = new ControllerConnector();
const controllerAsWalletStandard = controllerConnector.asWalletStandard() // type WalletWithStarknetFeatures
