...
Cartridge Controller implements a standard StarkNet account interface and can be seamlessly integrated into your application like any other wallet.
...
For more advanced use cases, you can also pass dynamic authentication methods:
...
For detailed implementation and examples, see the [headless authentication guide](/controller/headless-authentication.md).
...
:::info
Headless mode performs authentication in a hidden iframe without displaying any UI.
However, if session policies need approval or verification, the UI will open automatically to request user consent.
:::

When `connect()` is called, users will see an improved controller creation interface with username autocomplete functionality.
As users type their username, they'll see matching existing accounts with user profiles, making it easier to connect to existing controllers or choose unique usernames for new accounts.
...
:::info
Cartridge Controller will set **essential cookies** as part of the initialization.
...
This is suitable for simple applications or testing, but games typically benefit from using session policies for a smoother experience.
For complete details on session configuration, see the [sessions guide](/controller/sessions.md).
:::
...
The Controller SDK ships with sensible defaults for chain RPCs, but you can override them if needed.
For comprehensive configuration options and error handling, see the [configuration guide](/controller/configuration.md).
...
The Controller SDK provides an `open()` method for standalone authentication, which opens the keychain in first-party context.
This is useful for establishing first-party storage and enabling seamless iframe access across all games.
...
:::info
When using presets, make sure your preset is configured in the [@cartridge/presets](https://github.com/cartridge-gg/presets) repository.
See the [presets guide](/controller/presets.md) for more information.
:::

## Providers

Cartridge Controller is initialized through a "provider."
...
If the call matches the app's session policies, it signs automatically — no user prompt.
...
By default, Cartridge Controller exposes the `StarknetWindowObject` interface.
However, Cartridge Controller also supports the [get-starknet wallet standard](https://get-starknet.vercel.app/), enabling integration with libraries like `starknet-react` and `solid.js`.

You can use Cartridge Controller directly or through the ControllerConnector to cast to the wallet-standard compatible `WalletWithStarknetFeatures` interface:
...
// Or using a preset (recommended for verified games)
const sessionConnector = new SessionConnector({
  preset: "my-game",
  rpc: "https://api.cartridge.gg/x/starknet/mainnet",
  chainId: constants.StarknetChainId.SN_MAIN,
...
The MetaMask SDK has been removed in favor of the EIP-6963 standard.
If your application relied on MetaMask SDK-specific features:
...
The lodash dependency has been completely removed.
If you were importing lodash utilities from this package, you'll need to replace them with custom utilities or install lodash separately in your application.

## Development Workflow

If you're contributing to Cartridge Controller or running the examples locally, you have two development modes available:
...
- [React](/controller/examples/react.md) - Integration with `starknet-react`, hooks and components, state management
- [Svelte](/controller/examples/svelte.md) - Svelte stores and reactivity, component lifecycle, event handling  
- [Rust](/controller/examples/rust.md) - Native integration, error handling, async operations
...
- Set up [Multiple Authentication Methods](/controller/signer-management.md) for backup authentication
